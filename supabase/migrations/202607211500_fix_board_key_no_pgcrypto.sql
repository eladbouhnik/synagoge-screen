-- ensure_owner_synagogue used gen_random_bytes() (pgcrypto), which isn't
-- enabled on this project, so every signup/login bootstrap crashed with
-- "function gen_random_bytes(integer) does not exist". Swap it for
-- gen_random_uuid(), built into core Postgres (already used for every id
-- default in this schema) — no extension dependency.
...
begin
  ...
  new_key := 'bk-' || substr(replace(gen_random_uuid()::text, '-', ''), 1, 12);
